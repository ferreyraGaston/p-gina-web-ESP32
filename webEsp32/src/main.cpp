...
este ejemplo mostrará
  1. cómo usar y ESP 32 para leer pines
  2. crear una página web para que un cliente (navegador web, teléfono inteligente, televisión inteligente) se conecte
  3. enviar datos desde el ESP al cliente para actualizar SOLO datos modificados
  4. enviar datos desde la página web (como un control deslizante o presionar un botón) al ESP para decirle al ESP que haga algo

  Si no está familiarizado con HTML, estilo de página CSS y javascript, tenga paciencia, estas plataformas de código son
  no es intuitivo y la sintaxis es muy inconsistente entre plataformas

  Conozco 4 formas de actualizar una página web
  1. enviar toda la página: actualizaciones muy lentas, causa redibujados feos de la página y es lo que ve en la mayoría de los ejemplos
  2. envíe datos XML a la página web que actualizará solo los datos modificados: actualizaciones rápidas pero método más antiguo
  3. Cadenas JSON que son similares a XML pero con un método más nuevo
  4. actualizaciones muy rápidas de sockets web, pero no estoy seguro de que todo el soporte de la biblioteca esté disponible para ESP

  Yo uso XML aquí...

  opciones de compilación
  1. módulo de desarrollo esp32
  2. velocidad de carga 921600
  3. velocidad de la CPU 240 mhz
  velocidad de flash 80 mhz
  modo de flash qio
  flash tamaño 4mb
  esquema de partición predeterminado


  NOTA: si su ESP falla al programar, presione el botón BOOT durante la programación cuando el IDE esté "buscando el ESP".

  La Licencia MIT (MIT)

  código escrito por Kris Kasprzak
  ...
  A título personal, si desarrolla una aplicación o producto utilizando este código
  y gana millones de dólares, ¡me alegro por ti!

*/

#include <WiFi.h>       // standard library
#include <WebServer.h>  // standard library
#include "SuperMon.h"   // Archivo .h que almacena el código de su página html

// aquí publica páginas web en la intranet de su hogar, lo que facilitará la depuración de páginas
// ya que solo necesita actualizar el navegador en lugar de volver a conectarse al servidor web
#define USE_INTRANET

// reemplace esto con los parámetros de conexión a la intranet de su hogar
#define LOCAL_SSID "your_home_ssid"
#define LOCAL_PASS "your_home_passwrord"

// una vez que esté listo para salir en vivo, esta configuración es a la que se conectará su cliente
